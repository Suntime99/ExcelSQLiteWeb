...
public partial class Form1 : Form
{
    ...
    private async void InitializeWebView2()
    {
        try
        {
            ...
            webView21.CoreWebView2.NavigationCompleted += (_, __) =>
            {
                ...
                try
                {
                    var last = _recentFiles.FirstOrDefault()?.FullPath;
                    if (!string.IsNullOrWhiteSpace(last) && File.Exists(last))
                    {
                        OpenMainFileInternal(last, clearPrevious: true);
                    }
                }
                catch (Exception ex)
                {
                    System.Diagnostics.Debug.WriteLine("Auto open last file failed: " + ex.Message);
                }
            };
    ...
    private async void ExecuteDataCleansing(System.Text.Json.JsonElement data)
    {
        ...
        try
        {
            ...
            // 仅支持 SQLite：默认清洗“当前主表”，输出 Cleansed 表
            var sourceTable = MainTableNameOrDefault();
            const string targetTable = "Cleansed";
            ...
            string outputTarget = outputOptions.ValueKind != System.Text.Json.JsonValueKind.Undefined && outputOptions.TryGetProperty("target", out var ot) ? (ot.GetString() ?? "newfile") : "newfile";
