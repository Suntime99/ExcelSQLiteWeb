...
## 6.1 导出 Excel（统一样式口径）
本工程所有导出的 Excel（包括查询结果/统计结果/清洗结果/校验结果/分拆结果等），默认应用以下格式（除非用户显式选择“保留原样式/不格式化”）：
1) 字体：**微软雅黑**  
2) 字号：**9pt**  
3) 对齐：**整表单元格水平+垂直居中**（表头可加深色背景/加粗为可选增强）  
4) 行高：根据 9pt 字体设置合理默认（建议 18~20）  
5) 列宽：自动适配内容并设置合理上限（避免超长文本拉爆布局）；或“先按内容自适应，再对超长列截断/换行”  

> 说明：CSV 不支持样式；CSV 输出仅保证 UTF-8 编码与 Excel 兼容（必要时做“防科学计数法”保护）。
